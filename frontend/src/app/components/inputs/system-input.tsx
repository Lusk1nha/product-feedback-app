...
export function SystemInput<F extends FieldValues>(
  ...
  const { name, placeholder, onChange, control, disabled, input } = props;
...
function StyledInput(props: StyledInputProps) {
  const { error, message, className, ...rest } = props;

  return (
    <div className="w-full flex flex-col gap-y-2">
      <input
        className={cn(
          "w-full h-10 sm:h-12 bg-[#F7F8FD] border text-[#3A4374] text-[13px] sm:text-[15px] py-4 px-4 sm:px-6 rounded-[5px] focus:outline-none focus:ring-1",
          !error
